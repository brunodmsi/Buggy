...
import ChangeBugDescriptionService from './ChangeBugDescriptionService';
...
let changeBugDescription: ChangeBugDescriptionService;
let createBug: CreateBugService;
let createProject: CreateProjectService;

describe('ChangeBugDescription', () => {
  beforeEach(() => {
    ...
    changeBugDescription = new ChangeBugDescriptionService(fakeBugsRepository);
  });
  ...
  it('should be able to change a bug description', async () => {
    ...
    const newDescription = 'Na verdade a div está dando overflow na rota Y';

    await changeBugDescription.execute({
      bug_id: bug.id,
      description: newDescription,
    });

    expect(bug.description).toEqual(newDescription);
  });

  it('should not be able to change a description of non-existing bug', async () => {
    await expect(
      changeBugDescription.execute({
